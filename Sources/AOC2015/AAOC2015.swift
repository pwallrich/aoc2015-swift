...
@main
struct AOC2015: ParsableCommand, AsyncParsableCommand {
    ...
    func run() async throws {
        ...
        guard let part = AOC2015Core.Part(rawValue: part) else {
            throw Error.invalidPart
        }
        try await AOC2015Core.AOC2015(testInput: useTestInput).run(day: day, part: part)
    }
}
